...
function isPayloadFor(type, payload) {
  ...
  switch (type) {
    ...
    case "frame":
      return isString(payload.id) && isFiniteNumber(payload.t) && isString(payload.route) && isOneOf(payload.kind, ["gesture", "periodic", "composite"]) && isString(payload.jpeg_base64);
    case "mic":
