...
function buildAnchor(element, options) {
  return {
    route: options.route,
    selector: _chunkJC4AM4RQcjs.buildSelector.call(void 0, element),
    component: _chunkJC4AM4RQcjs.getComponentName.call(void 0, element),
...
function PinComposer({ point, target, initialValue = "", onSubmit, onCancel }) {
  ...
  const selector = target ? _chunkJC4AM4RQcjs.buildSelector.call(void 0, target) : null;
...
function parseRealtimeEvent(raw, context) {
  ...
  switch (type) {
    ...
    case "response.function_call_arguments.done": {
      ...
      if (!_chunkN4M47HUUcjs.isLiveToolName.call(void 0, name)) return { type: "error", message: `Unknown tool call: ${name || "(unnamed)"}` };
      return {
        type: "tool_call",
  ...
  try {
    response = await fetchImpl(`${options.endpoint}/mint`, {
      ...
      headers: {
        ...
        [_chunkN4M47HUUcjs.LIVE_SESSION_HEADER]: options.sessionId,
...
function fillerEnvelope(sessionId, seq, t) {
  return { schema_version: _chunkN4M47HUUcjs.LIVE_SCHEMA_VERSION, session_id: sessionId, seq, t, type: "stream_state", payload: { state: "buffering" } };
}
...
var UnsentQueue = class _UnsentQueue {
  ...
  async nextBatch(limit = Number.POSITIVE_INFINITY) {
    ...
    for (const entry of this.entries) {
      ...
      if (envelopes.length > 0 && bytes + size > _chunkN4M47HUUcjs.LIVE_EVENTS_BODY_MAX_BYTES) break;
...
var StreamClient = class {
  ...
  headers(extra = {}) {
    return {
      ...
      [_chunkN4M47HUUcjs.LIVE_SESSION_HEADER]: this.options.sessionId,
      ...extra
    };
  }
...
function clearStoredBootstrap(storage = defaultStorage()) {
  try {
    _optionalChain([storage, 'optionalAccess', _157 => _157.removeItem, 'call', _158 => _158(LIVE_BOOTSTRAP_STORAGE_KEY)]);
  } catch (e16) {
  }
}
function bootstrapLiveToken(options = {}) {
  ...
  try {
    _optionalChain([storage, 'optionalAccess', _159 => _159.setItem, 'call', _160 => _160(LIVE_BOOTSTRAP_STORAGE_KEY, JSON.stringify(bootstrap))]);
  } catch (e18) {
  }
  return bootstrap;
}
...
var UnitStore = class _UnitStore {
  ...
  update(id, patch) {
    ...
    const wantsContent = patch.statement !== void 0 || (_nullishCoalesce(_optionalChain([patch, 'access', _161 => _161.anchors_add, 'optionalAccess', _162 => _162.length]), () => ( 0))) > 0;
...
var LiveSession = class _LiveSession {
  constructor(options, persisted) {
    ...
    this.storage = options.storage === void 0 ? defaultStorage2() : options.storage;
    ...
    if (persisted) {
      ...
      this.token = _nullishCoalesce(_optionalChain([bootstrap, 'optionalAccess', _163 => _163.token]), () => ( null));
      this.endpointOrigin = _nullishCoalesce(_nullishCoalesce(_optionalChain([bootstrap, 'optionalAccess', _164 => _164.endpoint]), () => ( options.endpoint)), () => ( null));
      this.startedAt = this.now();
      this.mode = _nullishCoalesce(options.mode, () => ( _chunkN4M47HUUcjs.DEFAULT_EXECUTION_MODE));
    ...
    if (canStream) {
      const queueOptions = {
        onStoreError: (error) => _optionalChain([options, 'access', _165 => _165.onError, 'optionalCall', _166 => _166(error)]),
      ...
      this.client = new StreamClient({
        ...
        onError: (error) => _optionalChain([options, 'access', _167 => _167.onError, 'optionalCall', _168 => _168(error)]),
  ...
  get sequence() {
    return { nextSeq: this.nextSeq, ackedSeq: this.ackedSeq, queueLength: _nullishCoalesce(_optionalChain([this, 'access', _169 => _169.queue, 'optionalAccess', _170 => _170.length]), () => ( 0)) };
  }
  ...
  suspend() {
    ...
    _optionalChain([this, 'access', _171 => _171.client, 'optionalAccess', _172 => _172.close, 'call', _173 => _173()]);
    this.detachPageHide();
    this.notify();
  }
  get isSuspended() {
  ...
  async stop() {
    ...
    this.emitter.dispose();
    for (const waiter of this.ackWaiters.splice(0)) waiter.resolve(false);
    _optionalChain([this, 'access', _174 => _174.client, 'optionalAccess', _175 => _175.close, 'call', _176 => _176()]);
    this.detachPageHide();
    this.clearStorage();
  ...
  recordUnit(input) {
    ...
    const firstAnchorT = _nullishCoalesce(_optionalChain([input, 'access', _177 => _177.anchors, 'access', _178 => _178[0], 'optionalAccess', _179 => _179.t]), () => ( this.elapsed()));
    const unit = {
      id,
      ...
      evidence: {
        frame_ids: _nullishCoalesce(_optionalChain([input, 'access', _180 => _180.evidence, 'optionalAccess', _181 => _181.frame_ids]), () => ( [])),
        annotation_ids: _nullishCoalesce(_optionalChain([input, 'access', _182 => _182.evidence, 'optionalAccess', _183 => _183.annotation_ids]), () => ( [])),
        transcript_span: _nullishCoalesce(_optionalChain([input, 'access', _184 => _184.evidence, 'optionalAccess', _185 => _185.transcript_span]), () => ( { t_start: firstAnchorT, t_end: this.elapsed() })),
        ..._optionalChain([input, 'access', _186 => _186.evidence, 'optionalAccess', _187 => _187.telemetry_window]) ? { telemetry_window: input.evidence.telemetry_window } : {},
        ..._optionalChain([input, 'access', _188 => _188.evidence, 'optionalAccess', _189 => _189.audio_clip_id]) ? { audio_clip_id: input.evidence.audio_clip_id } : {}
      },
      status: "initial"
    };
    this.units.add(unit);
  ...
  snapshot() {
    return {
      ...
      queueLength: _nullishCoalesce(_optionalChain([this, 'access', _190 => _190.queue, 'optionalAccess', _191 => _191.length]), () => ( 0)),
  ...
  async restoreFrameBytes(onError) {
    for (const entry of _nullishCoalesce(_optionalChain([this, 'access', _192 => _192.queue, 'optionalAccess', _193 => _193.all, 'call', _194 => _194()]), () => ( []))) {
      if (!_chunkN4M47HUUcjs.isLiveEnvelopeOfType.call(void 0, entry, "frame") || entry.payload.dropped) continue;
    ...
    await Promise.all(
      this.frames.filter((frame) => !frame.dropped && !this.frameBytes.has(frame.id)).map(async (frame) => {
        try {
          ...
          _optionalChain([onError, 'optionalCall', _195 => _195(error)]);
        }
      })
    );
  }
  emit(type, payload) {
    const envelope = {
      schema_version: _chunkN4M47HUUcjs.LIVE_SCHEMA_VERSION,
      session_id: this.id,
      seq: this.nextSeq++,
      t: this.elapsed(),
      type,
    ...
    if (this.client && this.phase === "running") {
      ...
      _optionalChain([this, 'access', _196 => _196.queue, 'optionalAccess', _197 => _197.enqueue, 'call', _198 => _198(envelope)]);
    }
    this.persist();
    this.notify();
    return envelope;
  ...
  frameKind(frameId) {
    return _nullishCoalesce(_optionalChain([this, 'access', _199 => _199.frames, 'access', _200 => _200.find, 'call', _201 => _201((frame) => frame.id === frameId), 'optionalAccess', _202 => _202.kind]), () => ( null));
  }
  ...
  handleEnded(reason) {
    ...
    this.streamStatus = "ended";
    this.emitter.dispose();
    _optionalChain([this, 'access', _203 => _203.client, 'optionalAccess', _204 => _204.close, 'call', _205 => _205()]);
    this.detachPageHide();
    for (const waiter of this.ackWaiters.splice(0)) waiter.resolve(false);
    this.clearStorage();
...
function defaultGetUserMedia(constraints) {
  if (typeof navigator === "undefined" || !_optionalChain([navigator, 'access', _206 => _206.mediaDevices, 'optionalAccess', _207 => _207.getUserMedia])) {
...
var LiveRuntime = class {
  constructor(options) {
    ...
    this.annotation = (annotation) => {
      ...
      _optionalChain([active, 'access', _208 => _208.interviewer, 'optionalAccess', _209 => _209.announceDrawing, 'call', _210 => _210({
  ...
  setMuted(muted) {
    const active = this.active;
    if (_optionalChain([active, 'optionalAccess', _211 => _211.interviewer])) {
      active.interviewer.setMuted(muted);
      return;
    }
    _optionalChain([active, 'optionalAccess', _212 => _212.microphone, 'optionalAccess', _213 => _213.setMuted, 'call', _214 => _214(muted)]);
  ...
  setPaused(paused) {
    this.paused = paused;
    if (paused) _optionalChain([this, 'access', _215 => _215.active, 'optionalAccess', _216 => _216.evidence, 'access', _217 => _217.pause, 'call', _218 => _218()]);
    else _optionalChain([this, 'access', _219 => _219.active, 'optionalAccess', _220 => _220.evidence, 'access', _221 => _221.resume, 'call', _222 => _222()]);
  }
  ...
  suspend() {
    ...
    if (active) {
      ...
      _optionalChain([active, 'access', _223 => _223.microphone, 'optionalAccess', _224 => _224.stop, 'call', _225 => _225()]);
    }
  ...
  startCaptures(micStream, { share = true } = {}) {
    ...
    const screen = new (0, _chunkJC4AM4RQcjs.ScreenCapture)(this.capture.displayMedia, this.capture.displayMediaVideo, {
      segmentStore: createDefaultSegmentStore(),
      sessionId: session.id,
      onStreamEnded: () => {
        evidence.setDisplayStream(null);
        if (_optionalChain([this, 'access', _226 => _226.active, 'optionalAccess', _227 => _227.screen]) === screen && session.status !== "ended") this.callbacks.onReshareNeeded(true);
      },
      onError: (error) => this.callbacks.onError(toError(error))
    });
  ...
  releaseCaptures(active) {
    ...
    _optionalChain([active, 'access', _228 => _228.interviewer, 'optionalAccess', _229 => _229.stop, 'call', _230 => _230()]);
    _optionalChain([active, 'access', _231 => _231.connector, 'optionalAccess', _232 => _232.dispose, 'call', _233 => _233()]);
  ...
  async doStop() {
    ...
    let voiceBlob = null;
    const events = _nullishCoalesce(_optionalChain([active, 'optionalAccess', _234 => _234.events]), () => ( []));
    if (active) {
      this.releaseCaptures(active);
      ...
      _optionalChain([active, 'access', _235 => _235.microphone, 'optionalAccess', _236 => _236.stop, 'call', _237 => _237()]);
    }
...
function defaultGetUserMedia2(constraints) {
  if (typeof navigator === "undefined" || !_optionalChain([navigator, 'access', _238 => _238.mediaDevices, 'optionalAccess', _239 => _239.getUserMedia])) {
...
function countByStatus(units) {
  const counts = Object.fromEntries(_chunkN4M47HUUcjs.UNIT_STATUSES.map((status) => [status, 0]));
...
function EndedCard({ units, reason, residualHint, onDismiss }) {
  ...
  const shown = _chunkN4M47HUUcjs.UNIT_STATUSES.filter((status) => counts[status] > 0);
...
function ModeSwitch({ mode, pendingMode, onChange, disabled = false }) {
  return /* @__PURE__ */ _jsxruntime.jsxs.call(void 0, "div", { "data-riffrec-mode-switch": "", style: { display: "inline-block" }, children: [
    /* @__PURE__ */ _jsxruntime.jsx.call(void 0, "div", { role: "radiogroup", "aria-label": "Execution mode", style: { ...groupStyle, opacity: disabled ? 0.56 : 1 }, children: _chunkN4M47HUUcjs.EXECUTION_MODES.map((option, index) => {
      const selected = option === mode;
      const last = index === _chunkN4M47HUUcjs.EXECUTION_MODES.length - 1;
...
function LiveOverlay({
  ...
  _react.useEffect.call(void 0, () => {
    ...
    setEndedReason(null);
    setDismissed(false);
    if (uncontrolledPausedRef.current) _optionalChain([onPauseChangeRef, 'access', _240 => _240.current, 'optionalCall', _241 => _241(false)]);
  ...
  const handleConsent = _react.useCallback.call(void 0, 
    (result) => {
      ...
      session.start();
      _optionalChain([onConsent, 'optionalCall', _242 => _242(result)]);
    },
    [session, onConsent]
  );
  const handleDecline = _react.useCallback.call(void 0, () => {
    session.declineConsent();
    _optionalChain([onDecline, 'optionalCall', _243 => _243()]);
  }, [session, onDecline]);
  ...
  const togglePause = _react.useCallback.call(void 0, () => {
    ...
    _optionalChain([onPauseChangeRef, 'access', _244 => _244.current, 'optionalCall', _245 => _245(next)]);
  }, [paused, controlledPaused]);
  ...
  const handleConfirmations = _react.useCallback.call(void 0, 
    async (confirmations) => {
      ...
      try {
        ...
        _optionalChain([onFinished, 'optionalCall', _246 => _246(result)]);
      } finally {
...
function LiveMount({
  ...
  const handlePause = _react.useCallback.call(void 0, (paused) => _optionalChain([runtimeRef, 'access', _247 => _247.current, 'optionalAccess', _248 => _248.setPaused, 'call', _249 => _249(paused)]), []);
  ...
  return /* @__PURE__ */ _jsxruntime.jsxs.call(void 0, _jsxruntime.Fragment, { children: [
    /* @__PURE__ */ _jsxruntime.jsx.call(void 0, 
      ...
      {
        session,
        profile: _optionalChain([runtime, 'optionalAccess', _250 => _250.consentProfile]),
        endpointOwner: config.endpointOwner,
        drawShortcut: config.drawShortcut,
        getUserMedia,
        onConsent: (result) => _optionalChain([runtime, 'optionalAccess', _251 => _251.consent, 'call', _252 => _252(result)]),
        onAnnotation: _optionalChain([runtime, 'optionalAccess', _253 => _253.annotation]),
        onFinished: (result) => _optionalChain([runtime, 'optionalAccess', _254 => _254.finished, 'call', _255 => _255(result)]),
...
//# sourceMappingURL=LiveOverlay-3XCBPPMW.cjs.map