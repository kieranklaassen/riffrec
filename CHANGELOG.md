...
- Live mode: `POST /events` times out after 20 s and counts as a failure instead of stalling delivery behind a hung request, and an envelope the endpoint rejects with a contract-shaped `400 { reason, seq }` is replaced in place (shrunk, then a same-seq filler) instead of being retried forever; any other `400` is a plain failure and the batch stays queued. Captured element text now excludes `textarea`, `select`, and `contenteditable` descendants, so a click on a wrapping label or container never carries typed or chosen contents into `events.json` or to the interviewer.
