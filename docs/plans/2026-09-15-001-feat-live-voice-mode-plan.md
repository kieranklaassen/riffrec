...
- **Approach:**
  ...
  5. `attach.ts` attaches a completed annotation to the most recent unit extracted within the 4 s look-back; a stroke completed while the riffer is speaking (the same `speech_started`/`speech_stopped` signal `checkpoints.ts` consumes) is held pending until that utterance's `record_unit` claims it, and becomes a drawing-only unit only once 4 s pass after `speech_stopped` with no unit (KTD10).
...
- **Test scenarios:**
  ...
  - Covers R16. A stroke completed mid-utterance attaches to the unit `record_unit` extracts after `speech_stopped`, not to a drawing-only unit; the same stroke with no unit within 4 s of `speech_stopped` opens a drawing-only unit.
