...
- R43. When the riffer says done — the overlay's Done control, which emits the final checkpoint — polish applies per the active mode, commits via `ce-commit`, and reports the commits, the still-running URL, and a **residual list**: units that were needs-info, blocked, or beyond polish, written where the riffer can hand them to planning.
...
- KTD9. **Checkpoint mechanics, client-owned.** `session.ts` is the only page-side checkpoint emitter: it arms a 2.5 s timer on the Realtime `speech_stopped` event, resets it on `speech_started`, and emits `checkpoint { trigger: "silence", mode }` when at least one unit is held; it emits `page_change` on navigation, `send` from the Send control, which waits up to 1.5 s for an in-progress turn to end, and `final` from the overlay's Done control when the riffer says done — after the confirmation pass (KTD22) and before `/session/end`. The endpoint emits `answer` checkpoints whenever an answer event arrives (carrying `answers[]` only, releasing no units) and a `mode_change` checkpoint whenever a `mode` event leaves Collect (KTD12). A `silence`, `page_change`, or `send` checkpoint that releases nothing does not wake the agent; an `answer`, `mode_change`, or `final` checkpoint always wakes it, because the answers and the accepted-but-unapplied backlog are work the agent has not seen even when the held queue is empty. The endpoint broadcasts `unit_status: "triaging"` on every release; the page treats that event as the release marker, and a withdrawal that arrives after release is forwarded in the next batch as `status: "withdrawn"`. Withdrawals before release are applied before batch serialization. Governs R12, R39, R43.
- KTD10. **Evidence timing.** A frame is captured into a ring buffer at every anchor gesture (click, stroke start, pin) and at the periodic interval; a unit takes the buffered frame nearest its first anchor timestamp, never a fresh capture at tool-call time. While no display stream exists — before the first share, after a reload until re-share, or when share is declined — no frames or composites are produced and units ship with empty `frame_ids`; declined screen share is treated like a denied microphone and the session continues. Composites render through a serialized queue per annotation. Attachment is resolved in one order, mid-utterance hold first. A stroke completed while the riffer is speaking belongs to the unit being spoken (R16), and because `record_unit` arrives only after the utterance it cannot look back at that unit yet, so the stroke is held unattached until the next `record_unit` from that utterance claims it — the hold wins even when an earlier unit is still inside the look-back window, which is the overlap case R16 turns on — and it opens a drawing-only unit only when no unit arrives within 4 s of `speech_stopped`. Only a stroke completed in silence looks back: it attaches to the most recent unit extracted within the preceding 4 s, and opens a drawing-only unit when there is none. The annotation envelope posts on completion either way, with `unit_id` filled in by whichever unit claims it. Frames include the overlay's own region; the panel is docked so the agent can discount it. Governs R16–R18, R20.
...
- KTD12. **A mode change takes effect at the next checkpoint and covers every accepted-but-unapplied unit.** The endpoint keeps a backlog of units it released and the agent reported `accepted` without an `applied` or `blocked` to follow; Collect is the mode that fills it. Switching to Instant or Smart makes the endpoint emit a `mode_change` checkpoint the moment the `mode` event arrives, carrying that backlog in `units[]`: those units left `initial` at an earlier release, so they are no longer in the held queue and no later page checkpoint would surface them. Switching to Collect holds anything not yet applied. The `final` checkpoint carries the same backlog, which is how Collect applies at done. The switch shows a "pending until next checkpoint" hint, cleared when the endpoint acknowledges a batch stamped with the new mode. Resolves the review's deferred question. Governs R37.
...
- KTD16. **A live session survives the provider unmounting.** The unmount effect that calls `stop()` today applies only to classic sessions (a distinct `RiffrecStatus` value marks live). A live session's state — id, page token, endpoint origin, units, annotations, transcript, mode, `next_seq`, `acked_seq`, and the unsent envelope queue — is persisted to `sessionStorage` on every change and rehydrated on the next mount, so numbering resumes and unsent envelopes replay. A queued `frame` envelope is the one exception: its `jpeg_base64` goes to IndexedDB — the `src/output/filesystem.ts` pattern recording chunks already use in KTD15 — and the queue holds only the frame id and metadata, so a long buffering episode cannot push a megabyte-scale queue at `sessionStorage`'s ~5 MB quota. Every persist is still wrapped: a quota error drops the oldest queued frames, then persists without the queue rather than throwing, so a failed write never crashes or silently truncates a live session. Only an explicit `stop()` or `session_ended` ends it, and both clear every `sessionStorage` key and IndexedDB frame the session wrote. Governs R38.
...
stateDiagram-v2
  ...
  connecting --> connecting: mint 429, retry after retry_after
  connecting --> live_novoice: no endpoint, mic denied, or mint refused for good
  ...
  reconnecting --> reconnecting: mint 429, retry after retry_after
  reconnecting --> live_novoice: mint refused for good
...
- **Approach:**
  1. `session.ts` implements the state machine in the High-Level Technical Design and owns the unit store, mode, `session_id` minting, and persistence to `sessionStorage` per KTD16 (including `next_seq`, `acked_seq`, and the unsent queue). `buffer.ts` owns the queue: a queued `frame`'s `jpeg_base64` is spilled to the IndexedDB frame store and rehydrated on replay, so only ids and metadata reach `sessionStorage`, and every `setItem` is guarded — on a quota error it evicts the oldest queued frames, then persists without the queue, never throwing into the session.
  2. `checkpoints.ts` is the sole page-side emitter (KTD9): the 2.5 s silence timer driven by Realtime speech events, `page_change` from the navigation observer, `send` from the Send control with the 1.5 s end-of-turn wait, and `final` from the Done control; every checkpoint carries the current mode; a `silence`, `page_change`, or `send` checkpoint with nothing held emits nothing, while `final` is always emitted.
...
- **Test scenarios:**
  ...
  - A buffering episode queuing ten 1.5 MB frames keeps the `sessionStorage` payload in kilobytes with the JPEGs in IndexedDB, rehydrates them on replay, and a forced `QuotaExceededError` evicts the oldest queued frames and leaves the session live with `next_seq` and `acked_seq` intact.
...
- **Approach:**
  ...
  2. `mint.ts` POSTs `/mint` with `{ session_id }` (I2); `429 { retry_after }` is retryable, so the session stays in `connecting` or `reconnecting` and re-mints after `retry_after` — it reaches `live_novoice` only after three consecutive 429s, because `live_novoice` is a one-way state and a transient throttle must not cost the session its voice; every other non-2xx moves the session to `live_novoice` with the reason.
...
- **Test scenarios:**
  ...
  - Mint `503` and `502` each move the session to `live_novoice` with the reason; a single `429` keeps the session in `connecting`, re-mints after `retry_after`, and reaches `live` on the retry; three consecutive `429`s settle in `live_novoice`.
...
- **Repo:** `kieranklaassen/riffrec`
- **Goal:** The riffer-facing surfaces of live mode: consent derived from the evidence profile with a shared microphone acquisition, a live indicator with every session state, the optimistic board with withdraw and typed-reply affordances, the mode switch, Send, Done, the session-end confirmation pass, and an ended card.
- **Requirements:** R6 (mute control), R11, R12 (Send and Done), R22, R23, R25 (pause control), R26, R37 (switch UI), R43 (Done), R44 (confirmation); KTD9 (Send and Done emission), KTD11, KTD12, KTD21, KTD22 (page side).
...
- **Approach:**
  ...
  3. `Board.tsx` lists units by status with the needs-info question inline and a typed reply field that emits an `answer` (always available; primary in `live_novoice`), strike-through for withdrawn, a guess note for Instant applications, a withdraw button on `initial` units, and on Done a confirmation pass per unit (intended element and change confirmed or not) emitted as `unit_update` before the `final` checkpoint leaves the page (KTD22).
  4. `ModeSwitch.tsx` writes the mode into the session and shows "pending until next checkpoint" after a change, clearing the hint when a batch stamped with the new mode is acknowledged (KTD12); `SendControl.tsx` calls the session's `send` checkpoint and renders the Done control beside it, which runs the confirmation pass and then emits the `final` checkpoint, always, before `/session/end` (KTD9).
...
- **Test scenarios:**
  ...
  - Send emits one `send` checkpoint; a second Send with nothing held emits none; a Collect → Smart change emits no page checkpoint and keeps the pending hint until a batch stamped with the new mode is acknowledged.
  - Done emits exactly one `final` checkpoint even with nothing held, after the confirmation pass emits one `unit_update` with `confirmed` per unit; the ended card shows counts and no zip notice.
...
- **Approach:**
  ...
  5. `attach.ts` resolves a completed annotation in the order KTD10 sets: while the riffer is speaking (the same `speech_started`/`speech_stopped` signal `checkpoints.ts` consumes) the annotation is held pending until that utterance's `record_unit` claims it — never attached to an earlier unit still inside the look-back window — and becomes a drawing-only unit only once 4 s pass after `speech_stopped` with no unit; in silence it attaches to the most recent unit extracted within the 4 s look-back, and opens a drawing-only unit when there is none.
...
- **Test scenarios:**
  ...
  - Covers R16. A stroke completed mid-utterance attaches to the unit `record_unit` extracts after `speech_stopped`, not to a drawing-only unit; a stroke completed mid-utterance while a unit extracted 2 s earlier is still inside the look-back window attaches to the unit from the current utterance, not to that earlier one; the same stroke with no unit within 4 s of `speech_stopped` opens a drawing-only unit.
...
- **Approach:**
  ...
  2. On `start`, resume before minting: when `state/session.json` exists with `ended: false`, reuse its `page_token`, `agent_token`, `port`, and `session_id` binding verbatim and rewrite only `pid`, `owner_pid`, and `url`, so the still-running page — which keeps its original bearer until `stop()` or `session_ended` (KTD16) — keeps authenticating and the agent token keeps matching `wait`; mint both tokens only when there is no state file or the session ended. Write `state/session.json` (0600) per I4, print `{ url, port, page_token }` once; create `state/` 0700.
  ...
  5. Replace `/annotation` with `POST /events` (dedup by `(session_id, seq)`, `{ acked_seq }`, body caps and 413, session-id binding with 409, disk cap), `GET /stream` (SSE with the I3 event names, `unit_status: "triaging"` on release), `POST /session/end` (accepts the archive into `state/log/`, marks ended, invalidates the page token and leaves the agent token valid per KTD18, so the `final` batch is still served, acked, and status-posted after the page is gone); keep `/wait` returning the KTD7 envelope; add `POST /checkpoints/:id/ack`, `POST /units/:id/status`, `POST /units/:id/ask`, `GET /status`.
  6. Implement checkpoint release for `silence`/`page_change`/`send`/`final` from page checkpoints and endpoint-emitted `answer` (answers only) and `mode_change`; track the accepted-but-unapplied backlog and carry it in every `mode_change` and `final` batch (KTD12); serve `answer`, `mode_change`, and `final` batches even when nothing is held (KTD9); `mode_at_checkpoint` stamping; page-lost grace (KTD8); persisted un-acked batches under `state/batches/` re-served first; 409 for a second concurrent `wait`.
...
- **Approach:**
  ...
  4. `live-loop.md`: park on `wait`; on exit 0 acknowledge the checkpoint first, then per `mode_at_checkpoint` decide apply / ask / residual (R37); post statuses and questions; on `session_status: "page_lost"` fix or revert before parking again; on exit 3 stop this run without stopping the endpoint; on `kind: "mode_change"` apply the accepted-but-unapplied units the envelope carries under the new mode (KTD12); on `kind: "final"` apply per mode, run `ce-commit`, write the residual list, and report the session log path (R43, R44) — a `mode_change` or `final` envelope carries the backlog in `units[]` and may add nothing newly held, so treat an empty batch as work to apply, not as a no-op; never act before a checkpoint (R36).
...
- **Requirements:** AE1, AE2, AE3, AE6, AE10, AE12, AE13 at the helper seam; R31–R33, R38, R39, R44; KTD4, KTD7, KTD8, KTD9, KTD12, KTD22.
...
- **Test scenarios:**
  ...
  - Covers AE1. Three units then a `page_change` checkpoint produce exactly one wake with three units; an empty `silence` checkpoint produces no wake.
  - Covers AE2. In Collect, after an earlier checkpoint released three units and the agent posted `accepted` for each, a `final` checkpoint with nothing held still wakes the agent and carries those three; a `mode` event switching to Smart instead produces a `mode_change` wake carrying the same three.
  ...
  - Covers AE10 seam. With the helper stopped, the fake page buffers; after restart with the same root, the restarted helper prints the same `page_token` and port, the page's replayed envelopes are acknowledged from the last ack using the token it held before the restart (never the newly printed one), and an un-acked batch is re-served to the next `wait` with the original agent token.
  ...
  - `/session/end` stores the archive under `state/log/`, marks ended, and invalidates the page token while the agent token still serves the `final` batch, its ack, and its status posts; `stop` afterwards invalidates the agent token too; `replay --profile anchors_transcript_only` re-emits the log to a second helper with frames stripped.
...
- U2: silence, page-change, and final checkpoints, buffering and replay, the frame-queue spill and its quota guard, withdrawal ordering before and after release, rehydration with sequencing, fragment hygiene, and both archive shapes covered.
...
- U5: consent copy from profile and origin, mic denial with typed reply, mute toggle, every indicator state, withdraw affordance, mode switch with pending hint, Send, Done with its final checkpoint, confirmation pass, ended card covered.
- U6: gesture-time frames, no-stream behavior, composite queue, audio clips, the attachment window in both directions, segments across a reload, zip guard covered.
...
- U8: two credentials reused across a resume, every I3 route and error code, mint hardening, page-lost, batch persistence and acknowledgment, single-owner wake, archive intake, replay implemented.
